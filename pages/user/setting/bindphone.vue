<template>
	<view class="page">
		<view class="title">{{ '绑定手机号' | lang }}</view>
		...
		<view class="container">
			...
			<view class="form">
				...
				<view class="form-item">
					...
					<view type="text" class="sendcode" size="small" @click="doSendSms"
						:disabled="sendSmsCountdown > 0">{{ sendSmsCountdown > 0 ? `${sendSmsCountdown}s ` + $lang('后可重发') : $lang('发送验证码') }}</view>
				</view>
				<view class="form-item">
				...
				<view class="form-item">
					<button class="btn btn-submit" @tap="doSubmit">{{ '确定提交' | lang }}</button>
				</view>
			</view>
		</view>

		<sendsms v-if="sendSmsShow" :phone="phone" type="bind" :sitecode="sitecode" @success="sendSmsSuccess" @close="sendSmsClose">
		</sendsms>
	</view>
</template>
...
<script>
	...
	export default {
		...
		onLoad(options) {
			...
			let sitecode = uni.getStorageSync('sitecode')
			this.setData({
